...
app.controller('ProfileController', ['$rootScope', '$scope', 'service', 'localStorageService', '$cordovaOauth', function($rootScope, $scope, service, localStorageService, $cordovaOauth) {
    ...
    $scope.musicOptions = [
        {icon: 'heart', name: 'បទចម្រៀងខ្ញុំចូលចិត្ត', desc: 'ចម្រៀងខ្ញុំចូលចិត្ត'},
//        {icon: 'cloud-download', name: 'បទចម្រៀងបានទាញយក', desc: 'ចម្រៀងបានទាញយក'},
        {icon: 'headphones', name: 'បទចម្រៀងសំរាប់អ្នក', desc: 'ចម្រៀងសំរាប់អ្នក'}
//        {icon: 'history', name: 'បទចម្រៀងបានស្ដាប់', desc: 'ចម្រៀងបានស្ដាប់'}
    ];
    
    ons.ready(function() {
        $scope.profile = {
            fid     : '',
            name    : 'តំនរភ្ជាប់ទៅកាន់ Facebook',
            gender  : '',
            email   : 'ដើម្បីបង្ហាញពត៌មានរបស់អ្នក',
            image_s : '',
            image_l : '',
            uuid    : 'ងាយស្រួលក្នុងការប្រើប្រាស់កម្មវិធី'
        };
    ...
    $scope.$on('refresh: loadProfile', function() {
        ...
        $scope.select = 'online';
        $scope.pageTitle = "កម្មវិធីចម្រៀងខ្មែរ";
    ...
    $scope.onClearCacheClick = function() {
        var success = function(status) {
            window.plugins.toast.showShortCenter('បានលុបទិន្នន័យដែលមិនចាំបាច់');
        };
        var error = function(status) {
            window.plugins.toast.showShortCenter('សូមព្យាយាមម្ដងទៀត...');
        };
    ...
    $scope.$on('ACTION_REFRESH_DONE', function() {
        if ($scope.progressing === 'start') {
            window.plugins.toast.showShortCenter('ទទួលបានទិន្នន័យថ្មីៗចុងក្រោយ');
        }
        $scope.progressing = 'stop';
    });
