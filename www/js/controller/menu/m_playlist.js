...
app.controller('MyPlaylistController', ['$rootScope', '$scope', 'service', 'localStorageService', function($rootScope, $scope, service, localStorageService) {
    ...
    $scope.$on('LOAD_PLAYLIST_DETAIL', function() {
        ...
        if (service.playlist_mode === 'online') {
            ...
            service.cloudAPI.liveMusicPlaylistDetailList( { playlist_id : service.selected_playlist.id, uuid: device.uuid } )
    ...
    $scope.onButtonClick_playlist = function(item, itemIndex) {
        if (navigator.connection.type === Connection.NONE) {
            var callback_offline = function(index) {
                if (index == 1) {
                    if (item.id === $scope.musicNow.id) {
                        window.plugins.toast.showShortCenter('មិនអាចលុចបានទេ...');
                    }
                    else {
            ...
            var options_offline = {
                'title' : item.title,
                'androidTheme' : window.plugins.actionsheet.ANDROID_THEMES.THEME_HOLO_LIGHT,
                'buttonLabels': ['Remove this Music', 'Add to Local Playlist..'],
                'androidEnableCancelButton' : true,
                'addCancelButtonWithLabel': 'Cancel',
                'position': [20, 40]
            };
        ...
        var callback = function(index) {
            if (index == 1) {
                if (item.id === $scope.musicNow.id) {
                    window.plugins.toast.showShortCenter('មិនអាចលុបបានទេ...');
                }
                else {
                    $scope.music_playlists.splice(itemIndex, 1);
                    window.plugins.toast.showShortCenter('បានលុបចេញពីកំរងបទចម្រៀង');
    ...
    $scope.onButtonClick_Offline = function(item, itemIndex) {
        var callback = function(index) {
            ...
            // Remove from Local Playlist
                ...
                if (buttonPlaylist === 'Remove from Local Playlist') {
                    $scope.musics.splice(itemIndex, 1);
                    $scope.showNoItem = ($scope.musics.length === 0) ? true : false;
                    ...
                    function (fileSystem) {
                        fileSystem.getFile(item.url, {create: false}, 
                            function (fileEntry) {
                                fileEntry.remove(
                                    function () {
                                        window.plugins.toast.showShortCenter('ការលុបបទចម្រៀងបានជោគជ័យ');
                                    }, 
                                    function (error) {}
                                );
                            }, 
        ...
        var buttonPlaylist = 'Remove from Local Playlist';
